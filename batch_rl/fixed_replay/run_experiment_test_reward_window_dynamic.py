...
class Elem(object):
    def __init__(self, snapshot, state, action_seqs, a, rho, re, no):
        global global_id
        self.gid = global_id
        global_id += 1
...
class TestRunner(object):
  ...
  def _create_directories(self):
    ...
    setup_logger('window reward cert', os.path.join(self._base_dir, 'test.log'))
    self._logger = logging.getLogger('window reward cert')
  ...
  def _update_action_seqs(self, action_seqs, actions):
    next_action_seqs = np.delete(action_seqs, 0, 0) if len(action_seqs) == self._max_window_size else action_seqs
    next_action_seqs = np.vstack((next_action_seqs, actions))
    return next_action_seqs
  ...
  def _compute_poison_sizes_tpa(self, actions, a_star, max_w):
    ...
    for a in range(self.num_actions):
      if a == action:
        ...
        poison_sizes[a] = self._compute_poison_size(action, a, n, c, W=max_w)
  ...
  def _compute_poisons_sizes_dtpa_helper(self, actions, cum_actions, w_prime, w_star, a_star, a_2, a_3, a_4, max_w):
  ...
  def _compute_poisons_sizes_dtpa(self, actions, cum_actions, w_prime, a_star, max_w):

    poison_sizes = np.ones(self.num_actions, dtype=np.int) * self._total_num

    poison_sizes[a_star] = 0
    ...
    for w_star in range(1, max_w+1):
      ...
      for a_3 in range(self.num_actions):
        ...
        min_poison_size = 1000
        a_4 = self._best_other_action_at_w(cum_actions, w_star, a_3, max_w)

        for a_2 in range(self.num_actions):
          if a_2 == a_star: continue

          poison_size = self._compute_poisons_sizes_dtpa_helper(actions, cum_actions, w_prime, w_star, a_star, a_2, a_3, a_4, max_w)
        ...
        poison_sizes[a_3] = min(poison_sizes[a_3], min_poison_size)

    return poison_sizes

  def _compute_action_and_certifications(self, actions, max_w):
    ...
    poison_sizes_1 = self._compute_poison_sizes_tpa(actions_arr[max_w-w_prime:], action, max_w=w_prime)

    poison_sizes_2 = self._compute_poisons_sizes_dtpa(actions_arr, cum_actions, w_prime, action, max_w=max_w)

    poison_sizes = np.minimum(poison_sizes_1, poison_sizes_2)

    self._logger.info(f'poison_sizes_1 = {poison_sizes_1}, poison_sizes_2 = {poison_sizes_2} --> poison_sizes = {poison_sizes}')

    return action, poison_sizes

  def take_action(self, state, action_seqs, rho_lim, re_cur, no):
      ...
      next_action_seqs = self._update_action_seqs(action_seqs, actions)
      self._logger.info(f'len(next_action_seqs) = {len(next_action_seqs)}')

      a_star, poison_sizes = self._compute_action_and_certifications(next_action_seqs, len(next_action_seqs))
      ...
      for a in range(self.num_actions):
          ...
          if rho <= rho_lim:
            ...
            elem = Elem(snapshot, state, next_action_seqs, a, rho, re_cur, no)
      ...
      return a_list, next_action_seqs
  ...
  def expand(self, state, action_seqs, rho_lim=0, re_cur=0, no=0):
    state_bytes = state.tobytes()
    ...
    a_list, next_action_seqs = self.take_action(state, action_seqs, rho_lim, re_cur, no)
    ...
    for a in a_list:
      ...
      next_dict = self.expand(next_state, next_action_seqs, rho_lim, re_cur+reward, no+1)
  ...
  def get_first(self, queue):
      elem = queue.pop(0)
      return elem.snapshot, elem.state, elem.action_seqs, elem.a, elem.rho, elem.re, elem.no, elem.gid
  ...
  def run_experiment(self):
    ...
    for cur_id in id_list:
      ...
      t = time.time()
      # net1_saver.restore(sess, f'/dev/shm/cropp_logs_test{cur_id}/checkpoints/tf_ckpt-49')
      # net1_saver.restore(sess, f'/home/ubuntu/data/cropp_logs/test{cur_id}/checkpoints/tf_ckpt-49')
    ...
    action_seqs = np.empty((0, self._total_num), dtype=np.int)

    self.expand(state, action_seqs, rho_lim=0, re_cur=0, no=0)
    ...
    while 1:
      while not cur_queue:
        ...
        self._logger.info(f'file queue-{global_rho}.pkl close, read queue of length={len(cur_queue)} from the file ')

        self.state_dict = LimitedSizeDict()
        self._logger.info(f'************************************************ clearing all state dict due to increase to rho={global_rho}')

      snapshot, state, next_action_seqs, a, rho, re, no, gid = self.get_first(cur_queue)
      ...
      self.expand(next_state, next_action_seqs, rho, re+reward, no+1)
